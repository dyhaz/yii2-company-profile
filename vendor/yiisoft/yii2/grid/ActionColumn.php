    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    ...
                    'aria-label' => Yii::t('yii', 'Lihat'),
                    'data-pjax' => '0',
                    'class' => 'btn btn-success btn-xs',
                    'onclick' => "location.href='$url'"
                ], $this->buttonOptions);
                return Html::button('<i class="fa fa-eye"></i>', $options);
            };
        }
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                ...
                return Html::button('<i class="fa fa-pencil"></i>', $options);
        ...
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    ...
                    'aria-label' => Yii::t('yii', 'Hapus'),
                    //'data-confirm' => Yii::t('yii', 'Yakin menghapus item ini?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                    'class' => 'btn btn-danger btn-xs',
                    'onclick' => "if(confirm('".Yii::t('yii', 'Yakin menghapus item ini?')."')) location.href='$url'"
                ], $this->buttonOptions);
                return Html::button('<i class="fa fa-trash-o"></i>', $options);
